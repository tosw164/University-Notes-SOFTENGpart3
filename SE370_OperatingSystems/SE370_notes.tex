...
LECTURE 3:
	MISSED UP TO POEWR TO THE PEOPLE

	Power to the people
		Used to use teletypewriters, but used CRT TVs after a certain point.
	...
	Time Sharing System
		...
		BATCH SYSTEM REMNANTS COPY

	1980s computers
		Cycle starts again, started with Resident Monitor Systems.
		Simple single layer file systems
		No security, everything stored on disks
			Didn't bother as it was aimed at individuals.
		Did spooling later as well for printer output.
		Putting more than one program in memory, using similar system to resident monitor.

		Xerox created GUI elements for Office use.
		Then Apple engineers used ideas to create a personal computer
	...
	1980s Networking
		...
		Network OS
			File sharing, communication scheme, running independent to other machines on network.

		Distributed OS
			[NOT SURE] using lots of computers to make it look like only one OS controlling system.

	Multiprocessor Systems
		Heat creating an issue, kind of a soft cap on processor frequency.
		Therefore just add more cores instead of trying to make it go faster.

		TIGHTLY COUPLED SYSTEM

		Parallel Systems
			Increased throughput and cheaper way to increase performance.
			Increased reliability and rate of degradation.

			Symmetric Multiprocessing
				every core running same OS
				Most modern systems run this way


			Asymmetric Multiprocess
				Different cores allocated to different jobs/section.
				Used in very large system


	Realtime System
		Timing constraints very important.
		HARD	- must run within time, or failure happens
				- Has to be specifically designed to be hard realtime
		SOFT	- Doesn't matter too much, more lax.
				- Most OS handle soft realtime

	Pocket Computer & Smartphones
