...
Lecture 7
	...
	Pre-emptive Multitasking
		OS using some kind of criteria to determine how large of a time slice that task
		...
		Cognitive multitasking
			threads know that may have thread.yield() called on it and therefore [SOMETHING]

	Cooperative Multitasking
		Approaches
			Process yields right to run
			System stop process when system call made

		Doesn't mean task won't run and complete in one go.

	UNIX and preemptive
		preemptive at user level always
		hasn't always been preempntive at system level
	...
	Context Switch
		Change from one process running to another on same processor
...
Lecture 8
	Scheduling Processes/Threads
		CPU burst time = time CPU takes to wait
